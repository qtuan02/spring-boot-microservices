...
import static com.qtuan02.order.testdata.TestDataFactory.*;
...
class OrderControllerUnitTest {
    ...
    @BeforeEach
    void setUp() {
        given(securityService.getLoginUserName()).willReturn("Tuan");
    }

    @ParameterizedTest(name = "[{index}]-{0}")
    @MethodSource("createOrderRequestProvider")
    void shouldReturnBadRequestWhenOrderPayloadIsInvalid(CreateOrderRequest request) throws Exception {
        given(orderService.createOrder(eq("Tuan"), any(CreateOrderRequest.class)))
                .willReturn(null);
